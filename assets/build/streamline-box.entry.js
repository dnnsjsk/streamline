import { r as registerInstance, h, e as Host, i as getElement } from './index-6c45a1d5.js';
import { s as state } from './internal-7b6bacde.js';
import { s as state$1 } from './local-0f161f71.js';
...
const streamlineBoxCss = ":host .focus{box-sizing:border-box;position:relative;display:flex}:host .focus input:focus,:host .focus button:focus{outline:none}:host .focus input{padding-left:var(--sl-p)}:host .focus:before{position:absolute;content:\"\";box-sizing:border-box;left:4px;top:4px;width:calc(100% - 8px);height:calc(100% - 9px);pointer-events:none}:host .focus:focus-within:before{box-shadow:inset 0 0 0 2px var(--sl-focus-color)}:host .focus.is-main:before{left:0;top:0;width:100%;height:100%}@keyframes fadein{from{opacity:0;transform:translate3d(-50%, calc(-50% + 20px), 0)}to{opacity:1;transform:translate3d(-50%, -50%, 0)}}:host .container{width:100%;height:100%;position:absolute;top:50%;left:50%;transform:translate3d(-50%, -50%, 0);max-width:700px;max-height:700px;z-index:99999999999;background-color:var(--sl-bg);border-radius:0;display:grid;overflow:hidden;animation:fadein 0.4s cubic-bezier(0.4, 0, 0.2, 1)}:host .inner{left:var(--sl-side-w);position:absolute;height:100%;width:calc(100% - var(--sl-side-w))}:host .search{height:calc(var(--sl-side-w) - 2px);font-size:1rem;width:100%;margin:0;padding:0;border:none;border-bottom:1px solid var(--sl-border-color)}";
...
const StreamlineBox = class {
  ...
  handleChange(event) {
    ...
    state.entriesActive =
      this.value === ''
        ? state.entriesMenu
        : filterDeep(state.entriesMenu.filter((val) => Object.keys(val).length !== 0), (o) => {
          return o.name.toLowerCase().includes(this.value.toLowerCase());
        }, { childrenPath: ['children'] });
  }
  render() {
    return (h(Host, null, h("div", { class: "container" }, h("streamline-sidebar", null), h("div", { class: "inner" }, h("div", { class: "focus" }, h("input", { part: "search", class: "search", type: "text", placeholder: "Search for anything", onInput: (event) => this.handleChange(event) })), state$1.active === 'menu' && h("streamline-entries", null)))));
  }
